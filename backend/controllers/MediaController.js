...
export const SaveMedia = async (req, res) => {
  try {
    ...
    const sharedMedia = new TVUploadedMedia({
      title: title,
      tvId: tvId,
      mediaUrl: mediaUrl,
    ...
    res
      .status(201)
      .json({ success: true, message: "Media saved successfully", mediaUrl });
  } catch (error) {
...
export const DeleteMedia = async (req, res) => {
  ...
  try {
    ...
    if (!mediaCollection)
      return res
        .status(404)
        .json({ success: false, message: "Media not founddd" });
    const isShared = await TVUploadedMedia.findById(_id);

    const result = await cloudinary.uploader.destroy(public_id, {
      resource_type: mediaType.toLowerCase(),
    });
    if (result.result !== "ok") {
      return res
        .status(400)
    ...
    if (isShared) {
      console.log("shared");
      await TVUploadedMedia.findByIdAndDelete(_id);
      const latestMedia = await TVUploadedMedia.findOne().sort({
        createdAt: -1,
      });
...
export const StoreMedia = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "Media stored successfully", mediaUrl });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
